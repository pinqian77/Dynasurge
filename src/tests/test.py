...
def simulation_fast(target_model : InferenceEngine, draft_model: InferenceEngine, prompts :list[str], tokenizer: AutoTokenizer,T=0.6, top_p=0.9,
...
def simulation_baseline(target_model : InferenceEngine, draft_model: InferenceEngine, prompts :list[str], tokenizer: AutoTokenizer,T=0.6, top_p=0.9,
            max_length=512, residual_graph=None, grow_map=None, sampling_callables = None,
            sample_gather_indices = None, vocab_size=32000):
...
def main(args):
    ...
    target_model = InferenceEngine(max_length=args.M, model_name_or_path = args.target, dtype = torch.float16, device="cuda:0", draft=False)
    if args.Mode == 'spec':
        draft_model = InferenceEngine(max_length=args.M, model_name_or_path = args.model, dtype = torch.float16, device="cuda:0")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--growmap', type=str, default="../../Sequoia/8x8-tree.pt", help='growmap path')
