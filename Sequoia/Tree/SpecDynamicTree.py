...
class SpecInferTree(Tree):
    ...
    def collective_grow_dynamic(self, idx_list: list[int], n_branch_list: list[int], benchmark=False, grow_step=None):
        ...
        new_tokens_set = torch.unique(sampling_q.multinomial(num_samples=max_branch, replacement=True),dim=).flatten()

        unique_tokens = torch.unique(new_tokens_set)
        total_branch = len(unique_tokens)
        self.tokens[self.num_nodes: self.num_nodes + total_branch] = unique_tokens
        ...
        self.draft_kv_len = self.num_nodes
        self.draft_logits[start_pos - self.ground_truth_len + 1:end_pos - self.ground_truth_len + 1] = \
    ...
    def verify(self, benchmark=False):
        ...
        self.draft_model_engine.engine.kv_cache.gather_kv_incremental(accept_list[self.ground_truth_len:],
                                                                      self.ground_truth_len)
        self.target_model_engine.engine.kv_cache.gather_kv_incremental(accept_list[self.ground_truth_len:],
                                                                       self.ground_truth_len)
    ...
    def construct_grow_map(self, grow_map, benchmark=False):
        ...
        for i in range(self.draft_step - 1):
            if benchmark:
                _, t1, t2 = self.collective_grow_dynamic(self.grow_map_roots_gpu[i], self.grow_map['branches'][i],
                                                        benchmark=benchmark, grow_step=i)
                sample_time += t1
                compute_time += t2
            else:
