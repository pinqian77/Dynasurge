...
from sampling.kvcache_model_with_spec_tree import KVCacheModel_tree
...
def speculative_sampling_spec_tree(prefix: torch.Tensor, approx_model: torch.nn.Module, target_model: torch.nn.Module,
    ...
    approx_model_cache = KVCacheModel_tree(approx_model, temperature, top_k, top_p)
    target_model_cache = KVCacheModel_tree(target_model, temperature, top_k, top_p)
    ...
    while prefix.shape[1] < T:
        ...
        x = approx_model_cache.generate(prefix, gamma, 'approx')
        _ = target_model_cache.generate(x, 1, 'target')
